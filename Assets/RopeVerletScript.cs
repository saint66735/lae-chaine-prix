...
public class RopeVerletScript : MonoBehaviour
{
    ...
    private Rigidbody startRigid;
    private Rigidbody endRigid;
    ...
    private float TotalNodeLength() {
        return linePositions.Aggregate((linePositions.First(),0f), (acc, x) => (x,acc.Item2 + Vector3.Distance(acc.Item1,x))).Item2;
    }
    private float AverageNodeLength() {
        return TotalNodeLength()/linePositions.Length;
    }
    ...
    private void applyForceConnectedRigidBodies() {
        ...
        startRigid.AddForce( dir * forceMagnitude);
        endRigid.AddForce((chainEnd-endAnchor.position).normalized * forceMagnitude);
    }
    ...
    private void FixedUpdate()
    {
        ...
        //applyForceConnectedRigidBodies();
        //nodeCollider.enabled = false;
        //Physics.IgnoreCollision(nodeCollider, startAnchor.gameObject.GetComponent<>());
        //nodeCollider.
